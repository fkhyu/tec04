...
for i in range(5):  # Add initial body segments
...
world_size = 600  # Total size of the world (centered at origin)
...
while running:
    ...
    if not (-world_size <= new_head_x <= world_size) or not (-world_size <= new_head_y <= world_size):
        ...
        score = pygame.font.SysFont("Arial", 25).render(f"Your score: {len(snake)}", True, (255, 255, 255))
